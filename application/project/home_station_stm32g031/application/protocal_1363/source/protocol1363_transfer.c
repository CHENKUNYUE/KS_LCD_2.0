...
void protocl1363_send_byte_to_array(comm_struct *com, uint8_t data) {
    if (com->tx_buff_cnt < COMM_TX_MAXLEN - 1) com->tx_buff[com->tx_buff_cnt++] = data;
}
...
uint8_t protocol1363_check_frame_lenth(comm_struct *com) {
    ...
    if (!(s_version == VERSION_PROTOCOL || s_version == VERSION_LD)) return com_ver_err;

    // checksum
    if (com->rx_num < 18) return com_err;

    s_checksum  = four_ascii_to_u16((com->rx_buff) + com->rx_num - 5);
    s_checksum2 = protocol1363_get_checksum((com->rx_buff) + 1, com->rx_num - 6);

    if (s_checksum != s_checksum2) return com_checksum_err;
    ...
    if ((((~s_lencheck1) + 1) & 0x0f) != s_lencheck2) return com_lchecksum_err;
    ...
    if ((com->rx_num) - 18 != (s_lenth & 0x0fff)) return com_err;
...
uint8_t protocol1363_get_frame_end(comm_struct *com) {
    ...
    if (com->rx_buff[0] == PROTOCOL_1363_SOI) {
        ...
        if ((com->com_no <= 4) ||
            ...
            /*APP_PRINT("RX  %02d      ", com->rx_num);
            for (int i = 0; i < com->rx_num; ++i) {
                 APP_PRINT("%c", (char) com->rx_buff[i]);
                //APP_PRINT("%02X ", com->rx_buff[i]);
            }
            APP_PRINT("\r\n");*/
